    ...
    public class ModConfigMenu : ModConfigurationMenu
    {
        ...
        public override float DoWindowContents( Rect canvas )
        {
            ...
            foreach ( var relation in DefDatabase<PawnRelationDef>.AllDefsListForReading )
            {
                ...
                ColorWrapper wrapper = new ColorWrapper( RelationsHelper.RELATIONS_COLOR[relation] );
                GUI.color = wrapper.Color;
                Rect colorRect = new Rect( canvas.width - 24f - rowMargin, curY, 24f, 24f );
                GUI.DrawTexture( colorRect, Resources.Solid );
                if ( Widgets.ButtonInvisible( colorRect ) )
                {
                    // keep a pointer in scope for callback
                    var currentRelation = relation;
                    Find.WindowStack.Add( new Dialog_ColorPicker( wrapper, delegate { RelationsHelper.RELATIONS_COLOR[currentRelation] = wrapper.Color; }, false, true ) { closeOnClickedOutside = true } );
                }
